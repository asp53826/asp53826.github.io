...
function linkedinBannerSvg(variant = "architecture") {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">${base}<path d="M600 72V340" stroke="#21344d" stroke-dasharray="3 9" opacity=".5"/><text x="640" y="112" class="mono label" fill="#55ddff">AARYAN PATEL / ENGINEERING OS</text><text x="640" y="166" class="sans" font-size="47" font-weight="700" letter-spacing="-2.3" fill="#edf5ff">SYSTEMS + ML INFRASTRUCTURE</text><text x="640" y="202" class="sans sub">Evidence-first engineering across storage, compilers, inference, and autonomy.</text><g transform="translate(640 252)"><path d="M22 38H880" stroke="#304764"/><path d="M22 38H880" stroke="#55ddff" stroke-opacity=".42" stroke-width="2" stroke-dasharray="7 12"/>${[["DATA","ingest","#55ddff"],["STATE","MVCC + Raft","#65e6a7"],["RETRIEVE","HNSW","#9f8cff"],["SCHEDULE","paged KV","#9f8cff"],["COMPILE","typed IR","#f2b84b"],["VERIFY","oracle + limit","#65e6a7"]].map(([title,sub,color],i)=>`<g transform="translate(${i*152} 0)"><rect width="140" height="76" rx="4" class="node"/><rect width="3" height="76" fill="${color}"/><circle cx="18" cy="21" r="4" fill="${color}" filter="url(#glow)"/><text x="30" y="25" class="mono nodeTitle" fill="#edf5ff">${title}</text><text x="15" y="51" class="mono nodeSub">${sub}</text></g>`).join("")}</g><text x="54" y="352" class="mono label" opacity=".34">MOBILE + DESKTOP PHOTO SAFE ZONE</text></svg>`;
}
